  ...
  return (
    <AppLayout>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8 animate-fade-in-up">
        <div>
          <h1 className={`text-2xl sm:text-3xl font-bold tracking-tight transition-colors duration-500 ${c(dark ? 'dark' : 'light').text}`}>Manager Dashboard</h1>
      ...
      {/* Stats */}
      {!loading && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8 animate-fade-in-up animate-delay-100">
          ...
          {filtered.map((ad, i) => (
            <div key={ad.id} className="animate-fade-in-up" style={{ animationDelay: `${200 + i * 60}ms` }}>
              <div className={`rounded-2xl p-4 sm:p-5 transition-all duration-300 cursor-pointer hover-lift ${
                dark
                  ? 'bg-neutral-900/60 border border-neutral-800 hover:border-amber-500/25 hover:shadow-[0_0_30px_rgba(217,160,50,0.06)]'
                  : 'bg-white border border-stone-200 hover:border-amber-300/50 shadow-sm hover:shadow-md'
