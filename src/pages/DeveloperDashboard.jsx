  ...
  return (
    <AppLayout fullWidth>
      <div className="max-w-[1400px] mx-auto">
        ...
        {/* Top Cards */}
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8 animate-fade-in-up animate-delay-100">
          <SectionCard title="Your API Key">
            <div className={`p-3 rounded-xl text-xs font-mono break-all border transition-all duration-300 ${
              dark ? 'bg-neutral-900 border-neutral-700 text-amber-400' : 'bg-stone-50 border-stone-200 text-amber-700'
            }`}>
              {apiKey}
            </div>
            <p className={`text-xs mt-2 ${dark ? 'text-neutral-500' : 'text-stone-400'}`}>
              Use this key as <code className="text-amber-500">X-API-Key</code> header or <code className="text-amber-500">?api_key=</code> param on public endpoints.
            </p>
          </SectionCard>
          ...
          <SectionCard title="Public API Endpoint">
            <div className="space-y-2 text-xs">
              ...
              <code className={`block p-2 rounded text-[10px] ${dark ? 'bg-neutral-900 text-neutral-400' : 'bg-stone-100 text-stone-600'}`}>
                {API_BASE}/public/ads/
              </code>
              <p className={dark ? 'text-neutral-400' : 'text-stone-500'}>
                Authenticate via <code className="text-amber-500">X-API-Key</code> or <code className="text-amber-500">?api_key=</code>
              </p>
            </div>
          </SectionCard>

          <SectionCard title="Developer Auth">
            <div className="space-y-2 text-xs">
              <p className={dark ? 'text-neutral-400' : 'text-stone-500'}>
                Use JWT token in <code className="text-amber-500">Authorization: Bearer</code> header for private endpoints.
              </p>
              <p className={dark ? 'text-neutral-400' : 'text-stone-500'}>
                Login: <code className="text-amber-500">POST {API_BASE}/auth/developer-login/</code>
              </p>
            </div>
          </SectionCard>
          ...
          <SectionCard title="Available Ads">
            <div className="flex items-baseline gap-2">
              <span className={`text-4xl font-bold ${dark ? 'text-amber-400' : 'text-amber-600'}`}>
                {loading ? '...' : adsList.length}
              </span>
              <span className={`text-sm ${dark ? 'text-neutral-500' : 'text-stone-400'}`}>approved campaigns</span>
            </div>
          </SectionCard>
        </div>
        ...
        <div className="mb-8 animate-fade-in-up animate-delay-150">
          <SectionCard title={
            ...
            {appsLoading ? (
              ...
              <div className="space-y-2">
                {apps.map((app) => (
                  <div key={app.id} className={`flex items-center justify-between p-3 rounded-xl border ${dark ? 'bg-neutral-800/30 border-neutral-700/50' : 'bg-stone-50 border-stone-200'}`}>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className={`text-sm font-semibold ${dark ? 'text-neutral-200' : 'text-neutral-800'}`}>{app.app_name}</span>
                      ...
                      <p className={`text-[10px] font-mono mt-0.5 ${dark ? 'text-neutral-600' : 'text-stone-400'}`}>API Key: {app.api_key}</p>
                    </div>
                    <div className="flex items-center gap-2 ml-3">
                      <button onClick={() => handleEditApp(app)}
                        className={`text-xs px-2 py-1 rounded-lg transition-colors ${dark ? 'text-neutral-400 hover:text-amber-400 hover:bg-neutral-700' : 'text-stone-500 hover:text-amber-600 hover:bg-stone-100'}`}>
                        Edit
                      </button>
                      <button onClick={() => handleDeleteApp(app.id)}
                        className="text-xs px-2 py-1 rounded-lg text-red-500 hover:bg-red-500/10 transition-colors">
                        Delete
                      </button>
                    </div>
                  </div>
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <p className={`text-[10px] mt-0.5 ${dark ? 'text-neutral-500' : 'text-stone-400'}`}>
                        Client: {ad.client_name} &middot; Created {formatDate(ad.created_at)}
                      </p>
