...
const features = [
  'Launch campaigns in under 5 minutes',
  'AI-powered video ad generation',
  'Precision geo-targeting across India',
    ...
    <div className={`min-h-screen flex transition-all duration-700 ${c(dark ? 'dark' : 'light').page}`}>
      ...
      <div className={`hidden lg:flex lg:w-1/2 relative flex-col justify-between p-10 xl:p-14 overflow-hidden ${
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          <div className={`absolute top-1/2 left-1/3 w-[300px] h-[300px] rounded-full blur-[140px] transition-colors duration-700 ${c(dark ? 'dark' : 'light').glowViolet}`} style={{ opacity: 0.4 }} />
        </div>
        ...
        <div className="relative z-10 flex-1 flex flex-col justify-center max-w-lg mx-auto">
          ...
          <h2 className={`text-3xl xl:text-4xl font-bold tracking-tight leading-tight transition-colors duration-500 ${c(dark ? 'dark' : 'light').text}`}>
            Manage your ads{' '}
            <span className="gradient-text">smarter, faster,</span> better.
          </h2>

          <p className={`text-sm mt-3 leading-relaxed transition-colors duration-500 ${c(dark ? 'dark' : 'light').textMuted}`}>
            Everything you need to create, manage, and optimize your ad campaigns — all in one place.
          </p>
          ...
          {/* Testimonial */}
          <div className={`mt-10 p-5 rounded-2xl border transition-all duration-500 ${
            dark ? 'bg-white/[0.03] border-white/[0.06]' : 'bg-amber-500/[0.04] border-amber-500/10'
          }`}>
            <p className={`text-sm italic leading-relaxed transition-colors duration-500 ${c(dark ? 'dark' : 'light').textMuted}`}>
              "AdForge transformed how we reach our audience. The geo-targeting is incredibly precise and the AI video generation is a game-changer."
            </p>
            <div className="flex items-center gap-3 mt-4">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                dark ? 'bg-amber-500/20 text-amber-400' : 'bg-amber-500/20 text-amber-700'
              }`}>RK</div>
              <div>
                <p className={`text-xs font-semibold transition-colors duration-500 ${c(dark ? 'dark' : 'light').text}`}>Rajesh Kumar</p>
                <p className={`text-[10px] transition-colors duration-500 ${c(dark ? 'dark' : 'light').textDim}`}>Marketing Director, BrandWave</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-4 sm:p-6 lg:p-10 relative">
        ...
        <div className="w-full max-w-md">
          <div className={`rounded-2xl p-8 sm:p-10 transition-all duration-500 ${
            dark
              ? 'bg-neutral-900/80 backdrop-blur-xl border border-amber-500/12 shadow-[0_0_60px_rgba(217,160,50,0.06)]'
            ...
            <div className="text-center mb-7">
              ...
              <h1 className={`text-2xl font-bold tracking-tight transition-colors duration-500 ${c(dark ? 'dark' : 'light').text}`}>Welcome Back</h1>
              <p className={`text-sm mt-1 transition-colors duration-500 ${c(dark ? 'dark' : 'light').textMuted}`}>Sign in to your ads manager account</p>
            </div>
              ...
              <form onSubmit={handleVerifyOTP} className="space-y-5">
                <div>
                  ...
                  <p className={`text-sm mb-4 flex items-center gap-2 transition-colors duration-500 ${c(dark ? 'dark' : 'light').text}`}>
                    ...
                    <button type="button" onClick={() => setStep('mobile')} className={`text-xs font-medium underline transition-colors ${c(dark ? 'dark' : 'light').textLinkMuted}`}>Change</button>
                  </p>
            ...
            <div className={`mt-8 pt-6 border-t transition-colors duration-500 ${c(dark ? 'dark' : 'light').border}`}>
              <p className={`text-xs text-center mb-3 transition-colors duration-500 ${c(dark ? 'dark' : 'light').textDim}`}>Don't have an account?</p>
