      ...
      <div className={`relative w-full max-w-md transition-all duration-500 ${step === 'otp' ? 'scale-[1.02]' : 'scale-100'}`}>
        <div className={`rounded-2xl p-8 transition-all duration-500 ${t('card')(dark)}`}>
          <div className="text-center mb-8">
            ...
            <h1 className={`text-2xl font-bold tracking-tight transition-colors duration-500 ${t('text')(dark)}`}>Welcome Back</h1>
            <p className={`text-sm mt-1 transition-colors duration-500 ${t('textMuted')(dark)}`}>Sign in to your account</p>
          </div>
            ...
            <form onSubmit={handleVerifyOTP} className="space-y-5">
              <div>
                ...
                <p className={`text-sm mb-3 transition-colors duration-500 ${dark ? 'text-neutral-300' : 'text-neutral-700'}`}>
                  {mobile}{' '}
                  <button type="button" onClick={() => setStep('mobile')} className={`underline transition-colors ${t('textLinkMuted')(dark)}`}>Change</button>
                </p>
          ...
          <div className={`mt-6 pt-6 border-t transition-colors duration-500 ${t('border')(dark)}`}>
            <p className={`text-xs text-center mb-3 transition-colors duration-500 ${t('textDim')(dark)}`}>Don't have an account?</p>
            <Link to="/register" className={`block w-full py-2.5 rounded-xl text-sm font-semibold text-center transition-all duration-300 ${t('btnGhost')(dark)}`}>
              Create New Account
            </Link>
          </div>
        </div>
